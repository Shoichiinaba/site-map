...
$(document).ready(function() {
    ...
    $('#status').on('change', function() {
        window.crud.ajax.url("<?php echo base_url('/Home/search'); ?>/" + $('#id-siteplan').val() +
            "?status=" + $(this).val() + "&tgl_start=" + $('#tgl-start').val() + "&tgl_end=" + $(
                '#tgl-end').val()).load();
    });
...
function filter_tgl() {
    $(function() {
        $('input[name="daterange"]').daterangepicker({
            ...
            if ($('#status').val() == 'UTJ' || $('#status').val() == 'DP' || $('#status').val() ==
                'Sold Out') {
                alert('ya');
                window.crud.ajax.url("<?php echo base_url('/Home/search'); ?>/" + $('#id-siteplan')
                    .val() +
                    "?status=" + $('#status').val() + "&tgl_start=" + $('#tgl-start').val() +
